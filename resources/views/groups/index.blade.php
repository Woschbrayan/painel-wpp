<x-app-layout>
    {{-- Cabeçalho com ícone Lucide --}}
    <x-slot name="header">
        <div style="display: flex; align-items: center; gap: 10px;">
            <i data-lucide="message-circle" class="icon" style="color: #2563eb;"></i>
    ...
    <div style="background: #fff; padding: 24px; border-radius: 8px; box-shadow: 0 1px 4px rgba(0,0,0,0.05); max-width: 1000px; margin: 0 auto;">
        {{-- Filtro de Instância --}}
        <form method="GET" action="{{ route('groups.index') }}" style="margin-bottom: 32px;">
            ...
            <div style="overflow-x: auto;">
                <table style="width: 100%; border-collapse: collapse; background: #fff;">
                    <thead style="background: #f3f4f6; text-transform: uppercase; font-size: 14px; color: #374151;">
                        <tr>
                            <th style="text-align: left; padding: 12px; border-bottom: 2px solid #e5e7eb;">Nome do Grupo</th>
                            <th style="text-align: left; padding: 12px; border-bottom: 2px solid #e5e7eb;">JID</th>
                            <th style="text-align: left; padding: 12px; border-bottom: 2px solid #e5e7eb;">Ações</th>
                        </tr>
                    </thead>
                    <tbody style="font-size: 14px; color: #374151;">
                        @foreach($groups as $group)
                            <tr style="border-bottom: 1px solid #e5e7eb; transition: background 0.2s;" onmouseover="this.style.background='#f9fafb'" onmouseout="this.style.background='#fff'">
                                <td style="padding: 12px;">{{ $group['name'] ?? '(Sem nome)' }}</td>
                                <td style="padding: 12px;">{{ $group['jid'] ?? '-' }}</td>
                                <td style="padding: 12px;">
                                    <a href="{{ route('messages.form') }}?number={{ $group['jid'] }}&instance={{ request('instance') }}"
                                        style="display: inline-block; background: #16a34a; color: white; padding: 6px 12px; font-size: 12px; font-weight: 600; border-radius: 6px; text-decoration: none;"
        ...
        @else
            <p style="color: #6b7280; margin-top: 20px;">Nenhum grupo encontrado para esta instância.</p>
        @endif
    </div>

    {{-- Ativa ícones Lucide se necessário --}}
    @push('scripts')
        <script>
            lucide.createIcons();
        </script>
